...
from .views.htmx_views.htmx_utils_views import (
    check_database,
    get_rfid,
    get_rfid_in_home,
    get_vehicle_status,
    get_form_owner_update,
    get_form_car_update,
    update_status,
    get_rfid_in_change,
)
...
urlpatterns = [
    ...
    path(
        "check-in/vehicle/<str:data>/",
        VehiclePageView.as_view(),
        name="vehicle-detail-view",
    ),
    path(
        "register/rfid/<str:data>/", RegisterDetailView.as_view(), name="register-rfid"
    ),
    path(
        "owner/vehicle/list/", OwnerVehicleListView.as_view(), name="owner-vehicle-list"
    ),
    path(
        "owner/vehicle/detail/<int:pk>/<int:car_pk>/",
        OwnerVehicleDetailView.as_view(),
    ...
    path(
        "change/rfid/<str:data>/<int:pk>/",
        ChangeRfidChangeView.as_view(),
        name="rfid-view",
    ),
]
...
htmx_urlpatterns = [
    ...
    path(
        "get-owner-form/<int:owner_pk>/<int:car_pk>/",
        get_form_owner_update,
        name="get-form-owner-update",
    ),
    path(
        "get-car-form/<int:owner_pk>/<int:car_pk>/",
        get_form_car_update,
        name="get-form-car-update",
    ),
    path(
        "update-status/<str:status>/<int:car_pk>/", update_status, name="update-status"
    ),
]
